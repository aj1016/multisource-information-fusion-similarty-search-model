...
train_data = df.loc[210: 261, :]

with open('process data/multi-source_hydrology_fusion.csv', 'w', newline='') as csvfile:
...
for i in train_data.index:
    ...
    list5 = []
    df_all = pd.concat((df[name].iloc[:210, :], df2), axis=0, join='outer')
    ...
    if k < 2:
        k += 1
        df_sim.to_csv('process data/multi-source_hydrology_fusion.csv', mode='a', index=False, header=True)
    else:
        df_sim.to_csv('process data/multi-source_hydrology_fusion.csv', mode='a', index=False, header=False)

print('Start saving the results......')
df_sim = pd.read_csv('process data/multi-source_hydrology_fusion.csv')
...
df_sim.to_csv('process data/multi-source_hydrology_fusion.csv', index=False)