...
# model_ft = initialize_model(10, feature_extract, use_pretrained=True)
# model_ft = initialize_model(50, feature_extract, use_pretrained=True)
model_ft = initialize_model(100, feature_extract, use_pretrained=True)
...
'''路径修改'''
# path = '../project/项目工程文件/LSTM_Siamese_project_file/数据集制作及模型参数训练/色斑图模型参数训练程序/checkpoint_similarity_space(01_100).pth'
# path = '../project/项目工程文件/LSTM_Siamese_project_file/数据集制作及模型参数训练/色斑图模型参数训练程序/checkpoint_similarity_space(01_100).pth'
path = '../project/项目工程文件/LSTM_Siamese_project_file/数据集制作及模型参数训练/色斑图模型参数训练程序/checkpoint_similarity_space(03_100).pth'
...
# 该方法返回了一个字典对象，其中存储了加载的模型参数。字典中包含了'models'键和它对应的值，这个值就是保存的模型参数。
# 使用['models']索引操作获取这个值，并将其赋给model_ft对象的参数中，这样就完成了模型参数的加载。
...
'''路径修改'''
# 删除文件夹下的原图片
# path = ['ResNet_image_space(1_200)_comparison\\', "figure_ResNet_space(1_200)_K\\"]
# path = ['ResNet_image_space(1)_comparison\\', "figure_ResNet_space(1)_K\\"]
# path = ['ResNet_image_space(01_100)_comparison\\', "figure_ResNet_space(01_100)_K\\"]
path = ['ResNet_image_space(03_100)_comparison\\', "figure_ResNet_space(03_100)_K\\"]
...
for i in range(210, 262):
    ...
    for g in range(0, 210):
        ...
        distance.append('{:.6f}'.format(euclidean_space_distance.item()))
    ...
    for k in range(1, 11):
        ...
        if k < 2:
            for m in range(3):
                ...
                '''路径修改'''
                # path_0 = "ResNet_image_space(1_150)_comparison\\{}".format(x_i_dir)
                # path_0 = "ResNet_image_space(1)_comparison\\{}".format(x_i_dir)
                # path_0 = "ResNet_image_space(01_100)_comparison\\{}".format(x_i_dir)
                path_0 = "ResNet_image_space(03_100)_comparison\\{}".format(x_i_dir)

                # path_1 = "ResNet_image_time_comparison\\{}".format(x_i_dir)
...
for k in range(0, 10):
    ...
    '''路径修改'''
    # 指定图片保存路径
    # figure_save_path = "figure_ResNet_space(1_150)_K"
    # figure_save_path = "figure_ResNet_space(1)_K"
    # figure_save_path = "figure_ResNet_space(01_100)_K"
    figure_save_path = "figure_ResNet_space(03_100)_K"
...
'''路径修改'''
# 创建文件对象
# data = open('评价结果/K_ResNet_space(1_150)_evaluation.csv', 'w', encoding='utf-8', newline='')
# data = open('评价结果/K_ResNet_space(1)_evaluation.csv', 'w', encoding='utf-8', newline='')
# data = open('评价结果/K_ResNet_space(01_100)_evaluation.csv', 'w', encoding='utf-8', newline='')
data = open('评价结果/K_ResNet_space(03_100)_evaluation.csv', 'w', encoding='utf-8', newline='')
...
'''路径修改'''
# 填入数据
# data = pd.read_csv('评价结果/K_ResNet_space(1_150)_evaluation.csv')
# data = pd.read_csv('评价结果/K_ResNet_space(1)_evaluation.csv')
# data = pd.read_csv('评价结果/K_ResNet_space(01_100)_evaluation.csv')
data = pd.read_csv('评价结果/K_ResNet_space(03_100)_evaluation.csv')
...
'''路径修改'''
# data.to_csv('评价结果/K_ResNet_space(1_150)_evaluation.csv', index=False)
# data.to_csv('评价结果/K_ResNet_space(1)_evaluation.csv', index=False)
# data.to_csv('评价结果/K_ResNet_space(01_100)_evaluation.csv', index=False)
data.to_csv('评价结果/K_ResNet_space(03_100)_evaluation.csv', index=False)