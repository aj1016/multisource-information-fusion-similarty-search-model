...
w = pd.read_csv("process data/ridge_weight data.csv", header=0)
...
data = open('evaluation results/ridge_evaluation.csv', 'w', encoding='utf-8', newline='')
csv_writer = csv.writer(data)
csv_writer.writerow(['K', "correlation coefficient", "Root mean square error"])
data.close()

data = pd.read_csv('evaluation results/ridge_evaluation.csv')
data["K"] = data_k
data["correlation coefficient"] = data_r
data["Root mean square error"] = data_rmse
data.to_csv('evaluation results/ridge_evaluation.csv', index=False)