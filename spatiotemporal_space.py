...
'''Path modification'''
path = 'model training parameters/checkpoint_similarity_space.pth'
...
if os.path.exists('spatiotemporal_image_space_comparison\\'):
    shutil.rmtree('spatiotemporal_image_space_comparison\\')
    ...
    for k in range(1, 6):
        ...
        if k < 2:
            for m in range(3):
                ...
                x_m_0 = Image.open(os.path.join(Config.testing_space_dir, x_m_dir))
                x_m_0 = x_m_0.convert("RGB")
                x_m_0 = photo_transforms(x_m_0)
                x_m_0 = x_m_0.unsqueeze(0)
                x_m_0 = x_m_0.cuda()

                concatenated_0 = torch.cat((x_i_0, x_m_0), 0)
                x_i_dir = x_i_dir.split('.')[0]

                path = "spatiotemporal_image_space_comparison\\{}".format(x_i_dir)
                imshow(torchvision.utils.make_grid(concatenated_0), m, x_i_dir, x_m_dir, path, '{}_Dissimilarity: {:.4f}'.format(m + 1, dissimilarity))
...
data = open('evaluation results/spatiotemporal_space_evaluation.csv', 'w', encoding='utf-8', newline='')
csv_writer = csv.writer(data)
csv_writer.writerow(['K', "correlation coefficient", "Root mean square error"])
data.close()

data = pd.read_csv('evaluation results/spatiotemporal_space_evaluation.csv')
data["K"] = data_k
data["correlation coefficient"] = data_r
data["Root mean square error"] = data_rmse
data.to_csv('evaluation results/spatiotemporal_space_evaluation.csv', index=False)