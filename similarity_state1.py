...
df = pd.read_csv('查找结果/第1次三峡区间查找结果.csv')

df["时间相似"] = ""
df_1 = pd.read_csv('查找结果/similarity_name.csv')
x1 =  df_1.iloc[:, 1]
lis_2 = []
#时间相似
for i in range(len(x1)):
    lis_2.append(100 - float(x1[i].split('%')[0]))#使其统一数字大小的含义，数字小的更相似

df["时间相似"]=lis_2


df["空间相似"] = ""
df_2 = pd.read_csv('查找结果/dissimilarity_sort1.csv')
x2 = df_2.iloc[:, 2]
lis_3 = []
# 空间相似
...
df=df[["name","相似度","累积雨量","降雨历时","最大48小时降雨","万三累积雨量",'前10天降雨量',"起涨流量","时间相似","空间相似","最大洪峰"]]
name3=["累积雨量","降雨历时","最大48小时降雨","万三累积雨量",'前10天降雨量',"起涨流量","时间相似","空间相似"]
...
df1.fillna(df1.mean(numeric_only=True), inplace=True)#替换存在缺失值的样本 填充平均值，numeric_only=True由于数据中含有日期列，所以仅数据进行运算
...
def strtime_to_datetime(list2):
    ...
    return list1
df1["开始时间"] = strtime_to_datetime(df1["开始时间"])
df1["结束时间"] = strtime_to_datetime(df1["结束时间"])
...
df1["时间相似"] ='0'
df1["空间相似"] = '0'
df1 = df1[["相似度","累积雨量","降雨历时","最大48小时降雨","万三累积雨量",'前10天降雨量',"起涨流量","时间相似","空间相似","最大洪峰"]]
...
#输入
    ...
    label = int(input("请输入:"))
    # df1["name"] = '%s'%(label)
    x = df1[name3].iloc[label:label + 1, :]
    a, b, c, d, e, f, g, h = x.loc[label]#.loc使用的是标签名称索引，而.iloc是根据标签位置索引
    print('输入样本数据如下:')
    print(df1.iloc[label:label + 1, :])
    print('---------------------------------------------------------------------------')

    df3 = pd.DataFrame(data=[[a, b, c, d, e, f, g, h]],columns=name3)
    print("输入场次过程如下：")
    print(df3)
    m=input("请输入查找最近邻的数目：")
...
#计算相似度
    list5 = []
    df_all = pd.concat((df[name3].iloc[:len(df),:], df3), axis=0, join='outer')# axis=0表示纵轴，方向从上到下，体现出行的增加或减少，outer在列的方向上进行外连接（即求并集）
    ...
    # 增加权重
    df_nnnormal[:, 0] = df_nnnormal[:, 0] * 2.5
    df_nnnormal[:, 2] = df_nnnormal[:, 2] * 1.5
    df_nnnormal[:, 3] = df_nnnormal[:, 3] * 1.5
    df_nnnormal[:, 7] = df_nnnormal[:, 7] * 0.5#色斑图
    ...
    # 求最小的f个元素对应行号
    N_samll_index = pd.DataFrame({'相似度1': df_nnnormal_sum}).sort_values(by='相似度1', ascending=True)
    ...
    df_sim["相似度1"] = N_small[0].tolist()#[0],DataFrame的列索引
    # df_sim["name"] = N_samll_index
    df_sim=df_sim[["name","相似度1","累积雨量", "降雨历时", "最大48小时降雨", "万三累积雨量", '前10天降雨量', "起涨流量", "时间相似", "空间相似","最大洪峰"]]
    # df_sim=df_sim.set_index("开始时间")
    #保存小数点设置
    list6 = ["相似度1", "时间相似", "空间相似"]
    for i in list6:
    ...
    print("查找相似过程如下：")
    print(df_sim)
    df_sim.to_csv('查找结果/第%s次三峡区间查找结果1.csv' % n,index = False)#index = False表示不写入索引
